<template>
  <article class="md:flex">
    <h2 class="content-date h-full mt-px">
      <a :href="`#${content.title}@${content.date}`">{{ content.date }}</a>
    </h2>
    <div class="content-block">
      <div class="feed-border"></div>
      <div class="feed-dot"></div>
      <h1
        :id="`${content.title}@${content.date}`"
...
.document {
  @apply max-w-none prose-h3:mb-4 prose-h3:text-base prose-h3:leading-6 prose-sm prose prose-pre:text-base prose-slate prose-a:font-semibold;
}
</style>
